...
export default function App() {
	...
	const fetchShips = async () => {
		...
		`);
		setShipDataList(response.data.ships);
	};
	...
	useEffect(() => {
		if (searchTerm.length > 0) {
			const result = shipDataList.filter((ship) => {
				return ship.name.toLowerCase().includes(searchTerm.toLowerCase());
			});
			setSearchResult(result);
		} else if (searchTerm === '') {
			setSearchResult(shipDataList);
		}
	}, [searchTerm, shipDataList]);
	return (
		<div className='App'>
			<svg
				fill='#fff'
				style={{ height: '20px', margin: '10 0 10 10' }}
				version='1.1'
				x='0px'
				y='0px'
				viewBox='0 0 400 50'>
          ...
          c6,0,8.1-1.7,9.1-5.8h-34.8V23.9z'></path>
				...
				<g className='letter_x'>
					<path
						className='fill-white'
						d='M287.6,18.6H273l17.2,12.6c2.5-1.7,5.4-3.5,8-5L287.6,18.6z'></path>
					<path
						className='fill-white'
						d='M308.8,34.3c-2.5,1.7-5,3.6-7.4,5.4l13,9.5h14.7L308.8,34.3z'></path>
				</g>
